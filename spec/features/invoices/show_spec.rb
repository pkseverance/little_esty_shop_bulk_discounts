...
RSpec.describe 'invoices show' do
  before :each do
    ...
    @ii_12 = InvoiceItem.create!(invoice_id: @invoice_1.id, item_id: @item_8.id, quantity: 6, unit_price: 6, status: 1)

    @discount1 = @merchant1.discounts.create!(quantity_threshold: 8, percent_discount: 50)
  ...
  it 'has a link to the applied bulk discount next to each item if applicable' do
    visit merchant_invoice_path(@merchant1, @invoice_1)
